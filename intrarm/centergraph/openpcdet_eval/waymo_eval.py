...
# TODO: direct copy, need to further analyze the code
import numpy as np
import pickle
    ...
    def waymo_evaluation(self, prediction_infos, gt_infos, class_name, distance_thresh=100, fake_gt_infos=True):
        ...
        with self.test_session(graph=graph) as sess:
            config = tf.compat.v1.ConfigProto(
                inter_op_parallelism_threads=1,    # 指定用于运行independent op的线程的最大数量
                intra_op_parallelism_threads=1,    # 指定用于运行一个op的线程的最大数量
                allow_soft_placement=True,          # 如果指定的设备不存在，允许TF自动选择一个存在且能运行的设备
            )
